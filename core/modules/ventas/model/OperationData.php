...
class OperationData {
	...
	public function addByProduccion($idProductoARestar,$cantidad){
		$sql = "insert into ".self::$tablename." (product_id,q,operation_type_id,sell_id,created_at) ";
		$sql .= "value ($idProductoARestar,$cantidad,3,1,$this->created_at )";
...
// partiendo de que ya tenemos creado un objecto OperationData previamente utilizamos el contexto
	...
	public static function getById($id){
		...
		while($r = $query[0]->mysql_fetch_array()){
			$data->id = $r['id'];
...
public static function getBycompraId($id){
		$sql = "select * from ".self::$tablename." where id=$id";
		$query = Executor::doit($sql);
		$found = null;
		$data = new OperationData();
